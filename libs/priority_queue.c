			 ...
			 void *heap_addr)
{
	...
	pq->priority_queue_heap = (struct pair *)heap_addr;
}
...
void priority_queue_push(struct priority_queue_meta *pq, void *push_data)
{
	...
	struct pair *temp = (struct pair *)push_data;
	while (now != 1) {
		if (less_than_pair(temp, &pq->priority_queue_heap[now >> 1])) {
...
void priority_queue_pop(struct priority_queue_meta *pq)
{
	...
	struct pair temp = pq->priority_queue_heap[pq->priority_queue_size];
	pq->priority_queue_size--;
	int64_t child = 2;
